...
/*
  ...
  - axios baseURL to http://localhost:5001/api
  - Summary cards (users/products/orders/revenue)
  - Recent orders table with inline status edit
  ...
  - Small orders-by-status bar chart (canvas, no external libs)
  - Attempts admin endpoints (/api/admin/...) and falls back to non-admin endpoints
*/
...
async function apiTryAdmin(method, path, data = null, config = {}) {
  // try /admin path first, then fallback to the original path
  try {
    const adminPath = `/admin${path}`;
    return await axios.request({ method, url: adminPath, data, ...config });
  } catch (err) {
    // if admin endpoint doesn't exist or returned 404/401, try non-admin path
    // (we only catch and fallback — still surface other errors upstream)
    try {
      return await axios.request({ method, url: path, data, ...config });
    } catch (err2) {
      // rethrow the original error if you want details; here combine both
      throw err2;
    }
  }
}
        ...
        {products.map((p) => (
          <div key={p._id} style={styles.productCard}>
            <div style={styles.productImageContainer}>
              {p.image ? <img src={p.image} alt={p.name} style={styles.productImage} /> : <div style={styles.noImage}>No Image</div>}
            </div>
